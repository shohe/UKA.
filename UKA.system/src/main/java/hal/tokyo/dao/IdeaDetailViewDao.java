...
public class IdeaDetailViewDao {
	...
	public IdeaDetailViewBean read(int postId) throws SQLException {
		...
		// TODO:円グラフ表示用のパーセンテージ出力処理を実装していません Fumiya Endo 150626
		this.preparedStatement = this.connection
				.prepareStatement("SELECT p.title, p.date, c.posting_content, u.image, u.name, d.department_name, u.profilecomment, DATEDIFF( DATE_ADD( (p.date), INTERVAL( SELECT t.terms_period FROM postings p, terms t WHERE t.terms_id = p.terms_id AND p.posting_id = 3 ) DAY), CURRENT_DATE() ) AS timelimit FROM postings p, posting_content c, users u, departments d WHERE p.posting_content_id = c.posting_content_id AND p.user_id = u.mailaddress AND u.department_id = d.department_id AND p.posting_id = ?");
